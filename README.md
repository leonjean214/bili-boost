...
**跨会话健康档案**（v1.3.0 起）。单看速度会选中"忽快忽慢"的源 —— 一台 70% 时候飞快、30% 超时的镜像，体验差于一台始终中等的。脚本把每台主机的成败记在 `localStorage`：

```
排序键（字典序，三级）：本轮是否成功 → 成功率分档(每 15% 一档) → 本轮速度
```
...
HUD 上点击「编码模块」一行轮换三态。探测结果缓存在 `localStorage`，换显卡或升级驱动后跑 `__biliBoost.重新探测AV1()` 刷新。
...
__biliBoost.主机健康      // 各镜像的成功率与均速（跨会话累计）
